...
async function run() {
  const githubToken = (0, inputs_1.resolveInputValue)(
    'github-token',
    process.env.GITHUB_TOKEN,
  );
  const slackWebhookUrl = (0, inputs_1.resolveInputValue)(
    'pr-summary-slack-webhook-url',
    process.env.PR_SUMMARY_SLACK_WEBHOOK_URL || process.env.SLACK_WEBHOOK_URL,
  );
  const llmProvider = (0, inputs_1.resolveInputValue)(
    'llm-provider',
    process.env.LLM_PROVIDER || constants_1.DEFAULT_LLM_PROVIDER,
  ).toLowerCase();
  const openaiApiKey = (0, inputs_1.resolveInputValue)(
    'openai-api-key',
    process.env.OPENAI_API_KEY,
  );
  const anthropicApiKey = (0, inputs_1.resolveInputValue)(
    'anthropic-api-key',
    process.env.ANTHROPIC_API_KEY,
  );
  const geminiApiKey = (0, inputs_1.resolveInputValue)(
    'gemini-api-key',
    process.env.GEMINI_API_KEY,
  );
  const openaiModel = (0, inputs_1.resolveInputValue)(
    'openai-model',
    process.env.OPENAI_MODEL || constants_1.DEFAULT_OPENAI_MODEL,
  );
  const anthropicModel = (0, inputs_1.resolveInputValue)(
    'anthropic-model',
    process.env.ANTHROPIC_MODEL || constants_1.DEFAULT_ANTHROPIC_MODEL,
  );
  const geminiModel = (0, inputs_1.resolveInputValue)(
    'gemini-model',
    process.env.GEMINI_MODEL || constants_1.DEFAULT_GEMINI_MODEL,
  );
  const lookbackHours = Number(
    (0, inputs_1.resolveInputValue)(
      'lookback-hours',
      process.env.LOOKBACK_HOURS || constants_1.DEFAULT_LOOKBACK_HOURS,
    ),
  );
  const timezone = (0, inputs_1.resolveInputValue)(
    'timezone',
    process.env.TIMEZONE || constants_1.DEFAULT_TIMEZONE,
  );
  const maxResults = Number(
    (0, inputs_1.resolveInputValue)(
      'max-results',
      process.env.MAX_RESULTS || constants_1.DEFAULT_MAX_RESULTS,
    ),
  );
  ...
  const rawConfigPath = (0, inputs_1.resolveInputValue)(
    'config-path',
  ...
  if (prs.length === 0) {
    const message = (0, slack_1.buildSlackMessage)({
      periodLabel: (0, summary_1.formatPeriodLabel)({
        since,
        until: now,
        timezone,
      }),
  ...
  const { includedPrs, excludedPrs } = (0, analysis_1.splitPrsByInclusion)(
    detailedPrs,
  );
  const domainCounts = (0, analysis_1.countTags)(
    includedPrs.map(pr => pr.domainTags),
  );
  const signalCounts = (0, analysis_1.countTags)(
    includedPrs.map(pr => pr.signalTags),
  );
  const periodLabel = (0, summary_1.formatPeriodLabel)({
    since,
    until: now,
    timezone,
  });
  ...
  const finalSummary =
    summaryText ||
    (0, summary_1.buildRuleSummary)({ includedPrs, domainCounts });
  const message = (0, slack_1.buildSlackMessage)({
    periodLabel,
