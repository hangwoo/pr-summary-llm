...
async function fetchMergedPrs({ owner, repo, githubApiUrl, githubToken, since, until, maxResults, }) {
    const items = [];
    let page = 1;
...
async function fetchPrDetails({ owner, repo, githubApiUrl, githubToken, number, }) {
    const url = `${githubApiUrl}/repos/${owner}/${repo}/pulls/${number}`;
    return githubRequest(url, githubToken);
}
async function fetchPrFiles({ owner, repo, githubApiUrl, githubToken, number, }) {
    const files = [];
    let page = 1;
    while (true) {
        const url = new URL(`${githubApiUrl}/repos/${owner}/${repo}/pulls/${number}/files`);
        url.searchParams.set('per_page', '100');
        url.searchParams.set('page', String(page));
        const data = await githubRequest(url.toString(), githubToken);
