...
function buildSlackMessage({ periodLabel, summaryText, domainCounts, includedPrs, excludedPrs, topPrCount, }) {
    const blocks = [];
    const maxItems = Number.isFinite(topPrCount) && topPrCount > 0 ? topPrCount : 10;
    blocks.push({
        type: 'header',
        text: {
            type: 'plain_text',
    ...
    const domainLine = (0, summary_1.formatCounts)(domainCounts, constants_1.DOMAIN_LABELS);
    if (domainLine) {
        blocks.push({
            type: 'section',
            text: {
    ...
    if (excludedPrs.length > 0) {
        const list = excludedPrs
            .slice(0, maxItems)
            .map(pr => `- <${pr.url}|#${pr.number} ${escapeSlack(pr.title)}> (제외됨)`)
            .join('\n');
        blocks.push({
            type: 'section',
            text: {
...
async function postSlackMessage(webhookUrl, payload) {
